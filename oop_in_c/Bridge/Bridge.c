...
/**
 * @brief Implementor: TV.
 *
 * Pattern Role: Implementor interface (abstraction).
 * Defines platform-specific operations (turnChannel).
 */
typedef struct TV TV;
/**
 * @brief Virtual Table for TV Implementor.
 *
 * Pattern Role: Implementor interface definition.
 * Function pointers for TV operations.
 */
...
void TV_ctor(struct TV* const me, int inch_) {
  static struct TVVtbl const vtbl = {.turnChannel = tv_turningChannel};
  me->vptr = &vtbl;
  me->inch = inch_;
}
...
void on(struct RemoteControl* const me) { tv_on(me->implementor); }
void off(struct RemoteControl* const me) { tv_off(me->implementor); }
...
void nextChannel(struct RemoteControl* const me_) {
  ...
  if (me->currentStation == 200)
    printf("can not set the next channel because it is max\n");

  else
    setChannel(&me->super, ++(me->currentStation));
}

void previousChannel(struct RemoteControl* const me_) {
  struct ConcreateRemote* const me = (struct ConcreateRemote* const)me_;
  if (me->currentStation == 100)
    printf("can not set the next channel because it is min\n");
...
void concreate_setChannel(struct RemoteControl* const me_, int channel) {
  ...
  if (channel <= 200 && channel >= 100) {
    ...
    printf(
        " can not set that channel because it is out of the range from 100 to "
        "200\n");
  }
}
...
int main() {
  ...
  struct TV* const tv[2] = {(struct TV* const)&sony, (struct TV* const)&rcaTv};
